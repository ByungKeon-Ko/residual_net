...
def inst_res_unit(input_x, index, filter_size, short_cut, stride):
	W_conv1	= weight_variable ( [3, 3, nCOLOR, filter_size], 'w_conv%d_%d'%(filter_size, index) )
	...
	W_conv2	= weight_variable ( [3, 3, nCOLOR, filter_size], 'w_conv%d_%d' %(filter_size, index+1) )
	...
	bn2 = tf.nn.batch_norm_with_global_normalization(z_bn2, batch_mean2, batch_var2, tf.constant(1.0), None, 1e-20, 1.0)
...
class ResNet () :
	def infer (self, n, short_cut ):
		...
		self.bn_intro = tf.nn.batch_norm_with_global_normalization(z_bn_intro, mean_intro, var_intro, tf.constant(0.0 ), tf.constant(1.0), 1e-20, True)

		self.h_conv_intro	= tf.nn.relu( self.bn_intro )

		# ----- 32x32 mapsize Convolutional Layers --------- #
		self.gr_mat1 = np.zeros([n, 8])		# Graph Matrix
		for i in xrange(n) :
			if i == 0 :
				self.gr_mat1[i][:] = inst_res_unit(self.h_conv_intro, 16, short_cut, 1 )
			else :
				self.gr_mat1[i][:] = inst_res_unit(self.gr_mat1[i-1][7], 16, short_cut, 1 )

		# ----- 16x16 mapsize Convolutional Layers --------- #
		self.gr_mat2 = np.zeros([n, 8])		# Graph Matrix
		for i in xrange(n) :
			if i == 0 :
				self.gr_mat2[i][:] = inst_res_unit(self.gr_mat1[n-1][7], 32, short_cut, 2 )
			else :
				self.gr_mat2[i][:] = inst_res_unit(self.gr_mat2[i-1][7], 32, short_cut, 1 )

		# ----- 8x8 mapsize Convolutional Layers --------- #
		self.gr_mat3 = np.zeros([n, 8])		# Graph Matrix
		for i in xrange(n) :
			if i == 0 :
				self.gr_mat3[i][:] = inst_res_unit(self.gr_mat2[n-1][7], 64, short_cut, 2 )
			else :
				self.gr_mat3[i][:] = inst_res_unit(self.gr_mat3[i-1][7], 64, short_cut, 1 )


		# ----- FC layer --------------------- #
		self.W_fc1		= weight_variable( [8* 8* 64, 10], 'net12_w_fc1' )
		self.b_fc1		= bias_variable( [10], 'net12_b_fc1')
		h_flat			= tf.reshape( gr_mat3[n-1][7], [-1, 8*8*64] )

		self.mean_fc, self.var_fc = tf.nn.moments( h_flat, [0] )
		self.bn_fc = tf.nn.batch_norm_with_global_normalization(h_flat, self.mean_fc, self.var_fc, None, None, 1e-20, 1.0)

		self.y_prob		= tf.nn.softmax( tf.matmul(self.bn_fc, self.W_fc1) + self.b_fc1 )
